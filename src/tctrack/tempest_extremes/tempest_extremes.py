...
class TETracker(TCTracker):
    ...
    def read_trajectories(self) -> list[Trajectory]:
        ...
        # We need time metadata for the calendar
    ...
    def _set_time_metadata(self) -> None:
        """Extract time metadata from the input files and set the attribute.

        Calendar, units, start time, and end time are extracted from the time
        coordinate of the search variable in the first input file. Assumes other
        files match.
        """
        variable_name = (
            self.detect_parameters.search_by_min
        ...
        fields = cf.read(
            self.detect_parameters.in_data,
