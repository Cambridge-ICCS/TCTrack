...
class TSTORMSTracker(TCTracker):
        ...
        try:
            with Dataset(input_u_file, "r") as nc_file:
                ...
                if unlimited_dim not in nc_file.variables:
                    errmsg = (
                        "Coordinate variable for unlimited dimension '{unlimited_dim}' "
                        "not found."
                    )
                    raise KeyError(errmsg)
    ...
    def _parse_tstorms_trav_line_to_point(
        ...
        if variable_names:
            ...
            return_vars.update(
                {f"var_{i}": float(value) for i, value in enumerate(line[:-4], start=1)}
            )
    ...
    def read_variable_metadata(self) -> None:
        ...
        var_outputs: list[dict] = [
            ...
            {
                "name": "atmosphere_upward_relative_vorticity",
                "std_name": None,
                "long_name": None,
        ...
        for output in var_outputs:
            ...
            self._variable_metadata[var_name] = {
                "standard_name": (
                    output["std_name"]
                    if output["std_name"]
                    else field.get_property("standard_name", var_name)
                ),
                "long_name": (
                    output["long_name"]
                    if output["long_name"]
                    else field.get_property("long_name", var_name)
                ),
