...
class TCTracker(ABC):
    ...
    # Private attributes
    _variable_metadata: dict[str, TCTrackerMetadata]
    _time_metadata: TCTrackerTimeMetadata
    _global_metadata: dict[str, str]
    ...
    def time_metadata(self) -> TCTrackerTimeMetadata:
        ...
        """
        if not hasattr(self, "_time_metadata"):
    ...
    def to_netcdf(self, output_file: str) -> None:  # noqa: PLR0915
        """
        ...
        if (
            not self.variable_metadata
            or not self.global_metadata
            or not self.time_metadata
        ):
            self.set_metadata()
