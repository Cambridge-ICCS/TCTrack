...
class TRACKParameters(TCTrackerParameters):
    ...
    """ The minimum start-to-end distance which tracks must travel."""
...
class TRACKTracker(TCTracker):
    ...
    def _get_tracking_inputs(self) -> list[str]:
        ...
        inputs.append("n") # No country map
        inputs.append("n") # No initialisation file
    ...
    def tracking(self):
        ...
        tracks. according to the parameters in the :attr:`track_parameters` attribute
        that were set when the :class:`TRACKTracker` instance was created.
