...
* :meth:`~TCTracker._parameters` to return a list of the parameters objects so that they
  are accessible by the base class.
* :meth:`~TCTracker._set_metadata` to set variable, and time metadata. Parameters should
  be added to global metadata in json format and the time and variable metadata should
  be set appropriately based on the input data and using the
  :class:`TCTrackerTimeMetadata` and :class:`TCTrackerMetadata` classes.
* :meth:`~TCTracker.read_trajectories` to read any outputs generated by the tracking
  ...
  into CF-compliant NetCDF output. If the algorithm doesn't produce an intermediate
  output file then this function can be left blank and the `trajectories` argument to
  :meth:`~TCTracker.run_tracker` should be provided instead.
