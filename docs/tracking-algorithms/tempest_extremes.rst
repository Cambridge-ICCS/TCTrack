...
:class:`StitchNodesParameters`, :attr:`~StitchNodesParameters.in_fmt` and
:attr:`~StitchNodesParameters.in_file` will be determined from
:class:`DetectNodesParameters`. Also, :attr:`~StitchNodesParameters.out_seconds` should
be left as the default value of ``False`` to enable proper conversion to the netCDF
output file.

It is recommended to set the :attr:`DetectNodesParameters.output_dir` parameter so that
the intermediate files generated by Tempest Extremes will be retained. Otherwise, if
there is an error or you wish to rerun part of the process you will have to fully start
over. However, if you do not wish to retain the intermediate files then
:attr:`DetectNodesParameters.output_dir` and :attr:`StitchNodesParameters.output_dir`
can be left as the default (``None``) to avoid saving them. They will instead be stored
in a temporary directory that lasts for only the lifetime of the :class:`TETracker`
instance.
