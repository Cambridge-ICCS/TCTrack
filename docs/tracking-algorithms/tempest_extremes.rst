...
:attr:`~TEStitchParameters.in_file` will be determined from
:class:`TEDetectParameters`. Also, :attr:`~TEStitchParameters.out_seconds` should
be left as the default value of ``False`` to enable proper conversion to the netCDF
output file.

It is recommended to set the :attr:`TEDetectParameters.output_dir` parameter so that
...
:attr:`TEDetectParameters.output_dir` and :attr:`TEStitchParameters.output_dir`
